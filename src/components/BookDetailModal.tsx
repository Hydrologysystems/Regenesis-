      ...
      <div className="relative w-full max-w-2xl bg-[#FFFFFF] border border-[#1A1A1A] p-8 sm:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.2)] text-[#1A1A1A] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-8">
          ...
          <div className="pt-6 border-t border-[#1A1A1A]/15">
            ...
            {notified ? (
              <div className="p-4 bg-[#FFFDF0] border border-[#D4AF37]/50 text-[#1A1A1A] text-xs flex items-center gap-2 font-inter font-medium">
                <CheckCircle className="w-4 h-4 text-[#D4AF37]" />
                <span>Success! You will receive VIP drop alerts for {book.title}.</span>
              </div>
            ) : (
              <form onSubmit={handleNotify} className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  type="submit"
                  className="btn-gold-slide h-12 px-8 text-xs uppercase tracking-[0.2em] font-inter font-medium flex items-center justify-center gap-2 cursor-pointer shrink-0"
                >
